// NetTetrisSer.cpp : Defines the entry point for the application.
//

#include "stdafx.h"
#include "winbase.h"

#include "NetTetrisSer.h"
...
#define		STAGE_WIDTH				(800)				//Solo 458, Net 800
                     ...
                     int       nCmdShow)
{
	...
	_tcscat(szProgName, _T("DoNetTetrisSer"));
	...
	LoadString(hInstance, IDC_NETTETRISSER, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_NETTETRISSER));

	TCHAR szSkinFile[MAX_PATH];
	GetModuleFileName(GetModuleHandle(NULL), szSkinFile, MAX_PATH);
	_tcsrchr(szSkinFile, '\\')[0] = 0;
	_tcsrchr(szSkinFile, '\\')[0] = 0;
	_tcscat(szSkinFile, _T("\\DoTetris\\skin\\NetStage.xml"));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_NETTETRISSER)/*(LPCTSTR)IDI_NETTETRISSER*/);
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
//	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_NETTETRISSER);
	wcex.lpszClassName	= szWindowClass;
	wcex.hIconSm		= LoadIcon(wcex.hInstance, MAKEINTRESOURCE(IDI_SMALL)/*(LPCTSTR)IDI_SMALL*/);
...
//	HWND hWnd;
	...
	hIcon = LoadIcon(hInstance, (LPCTSTR) MAKEINTRESOURCE(IDI_NETTETRISSER/*IconResourceArray[nIndexOfIcon]*/));
...
LONG OnTrayNotification(HINSTANCE hInstance, HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	switch(lParam)
	{
		case WM_RBUTTONDOWN:
		{
			...
			if(hMenu = LoadMenu(hInstance, (LPCTSTR) MAKEINTRESOURCE(IDC_NETTETRISSER)))
			{
